...
### 7.3.2 Si las formas de solución no existen pero deben agregarse
<ol type="I">
...
Ahora que las formas de solución están en su lugar es importante que actualice y confirme las propiedades de la forma de solución real. Seleccione el módulo Apilamiento de lixiviación en pilas en el menú Utilidad y, a continuación, en la pestaña Solución.

Te recomendamos que revises los datos de la tasa de la aplicación y el tiempo de lixiviación; estos dos puntos de datos se rellenarían desde el archivo de configuración del proyecto.

Haga clic en Actualizar solución (Update Solution). Si olvida ejecutar la solución actualizada, el modelo no llevará la información actualizada. 
...
#### <em>1.	Definiciones globales de inputs</em>
...
| Time Step    | El intervalo entre los cálculos del modelo, medido en días. Opencontour está configurado para que el paso de tiempo predeterminado sea 1, es decir, diario. El usuario tiene la capacidad de editar este campo y actualizarlo cada dos días (2), semanalmente (7), etc.  |
...
Esta característica permite al usuario ejecutar el modelo utilizando un Write State, es decir, hasta un cierto punto en el tiempo, luego hacer una pausa.  Cuando un modelo grande se ejecuta en una marca de tiempo específica ("Write State"), todos los parámetros de bloque se almacenan para que las ejecuciones simultáneas comiencen a partir de esa fecha. Esta característica se puede utilizar al combinar datos de apilamiento históricos y pronosticados, o al ejecutar iteraciones después de una fecha especificada determinada. 

El usuario puede guardar todas las entradas en cualquier momento durante el proyecto. Sin embargo, se recomienda que, si se ha utilizado el Write State, o si alguna entrada se actualiza y difiere de la configuración original del proyecto, el usuario exporta el archivo CVS y lo guarda. También recomendamos que analice cómo nombrar y guardar estos archivos con su equipo antes de guardar y cargar información actualizada.

Se pueden encontrar más detalles en el <span style="color:green"><em>Capítulo 7.8 </em>.</span>
...
Algunos proyectos tendrán datos climáticos. Si este es el caso de su proyecto, revise los siguientes pasos; de lo contrario, continúe con el <span style="color:green">paso 7.5</span>
...
## 7.6 Tasa de aplicación de tipo lixiviación (Leach Type App Rate)
...
El primer paso es asignar valores manuales a: ltp, imc, r_ar, ar y ar_v.
...
## 7.7 Ejecución del modelo de recuperación de lixiviación en pila
...
## 7.8 Uso de la función Write State y lectura para modelos más grandes

Algunos proyectos requerirán que se agreguen datos adicionales a un modelo grande existente.  Esta adición podría ser una actualización de apilamiento de lixiviación real mensual o semanal o puede ser un plan de apilamiento pronosticado. Cuando un modelo grande se ejecuta a una determinada marca de tiempo ("Write State"), todos los parámetros de bloque se almacenan para que las ejecuciones simultáneas comiencen a partir de esa fecha.  Ahora describiremos los pasos necesarios para agregar los datos al modelo. 

1. Ejecute el modelo actual usando la función "Write State": Una vez completada la calibración, determine la última fecha para la que desea ejecutar su modelo, con el 'Write State' marcado, ejecute el modelo (tenga en cuenta que esta fecha será la fecha de inicio de este archivo, en lugar de la 'pm' mínima.<br/><br/>Esto guardará el "estado" de cada CutterResult al final de la ejecución del modelo. Esto incluye propiedades de ahorro como flujo de salida, días de lixiviación, metal extraíble restante, metal acumulado realizado, flujos acumulativos aplicados / drenaje. Además, un archivo se guardará el archivo JSON; persistiendo cualquier dato histórico:<br/><br/>El usuario ahora puede usar este nuevo archivo para trabajos futuros, como el apilamiento de pronósticos cuando necesita crear más rellenos o puede continuar trabajando en esta instancia creando más formas de relleno.
2. Ya sea trabajando en esta instancia, o en una separada, el usuario ahora tendrá que verificar el Read State para decirle al modelo que retome donde lo dejó nuestra última ejecución de modelo. Note que cuando ejecutamos el modelo con el Read State marcado, aparece una línea roja punteada que marca la fecha del último modelo de ejecución. Además, mostramos las fechas en que el modelo se ejecutó por última vez en la parte superior de la pantalla (desde y hacia).<br/><br/>
...
## 7.9 Combinación de datos históricos con actualizaciones futuras (pronóstico)
...
1.	Utilice el nuevo archivo .json de grupo del modelo más grande como la topografía base inicial para este nuevo apilamiento para que no haya huecos.<br/><br/>
...
### 7.9.1 Importe el archivo CutterResult en el modelo existente: 
el nuevo conjunto de datos de apilamiento (CutterResult) se anexará al modelo existente.  

Seleccione la pestaña "Misc" en el menú Apilamiento de lixiviación para importar el archivo CutterResult ("Rehandled CutterResult").
...
Si los bloques se han vuelto a manipular para crear un nuevo espacio para el nuevo apilamiento, los bloques antiguos se eliminarán y se reemplazarán con el nuevo apilamiento.

![Image](./images/import_cutters.png)

Utilice la herramienta Script para rellenar la propiedad 'ltp' en la capa CutterResult.  Después de hacer clic en Aceptar, salga de la ventana.
...
De vuelta al menú Utilidad, seleccione Módulo de apilamiento y, en la pestaña Solución, realice la función "Actualizar solución" para rellenar la capa del modelo con los polígonos de solución más recientes. 

![Image](./images/update_solution.png)

Después de ejecutar el modelo (para obtener más información sobre cómo ejecutar el modelo, vaya al <span style="color:green"><em>Capítulo 7.7</em></span>), los nuevos datos serán acumulativos para todo el panel. Los datos se pueden exportar a Excel para su posterior análisis.